...
const getCookie = () => document.cookie.length && document.cookie.split('; ').find(row => row.startsWith('xiroadminSidebarState=')).split('=')[1];
...
const headerCondensedItems = [].slice.call(headerCondensedItemContainer.querySelectorAll('.header-dd-item')); // let headerTitleWidth = headerTitleArea.getBoundingClientRect().width;

let headerTitleWidth = 256;
...
window.addEventListener('joomla:menu-toggle', event => {
  headerItemsInDropdown();
  document.cookie = `xiroadminSidebarState=${event.detail};`;
