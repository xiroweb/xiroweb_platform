...
customElements.define('joomla-field-module-order', class extends HTMLElement {
  ...
  connectedCallback() {
    this.linkedFieldSelector = this.getAttribute('data-linked-field') || 'jform_position_id';
  ...
  writeDynaList(selectProperties, source, originalPositionName, originalPositionValue) {
    ...
    selectNode.id = selectProperties.id; // eslint-disable-next-line no-restricted-syntax
    ...
    for (const x in source) {
      ...
      const item = source[x]; // eslint-disable-next-line prefer-destructuring
